...
public class CollapseAnimation extends Animation {
	 ...
	 *            and set to gone
	 */
	public CollapseAnimation(View view, ListAdapter listAdapter, int position, AbstractSlideExpandableListAdapter slideAdapter) {
		...
		if (slideExpandableListView != null) {
			if (slideAdapter.getSavedParentInitialHeight() == 0) {
				AbstractSlideExpandableListAdapter sonAdapter = (AbstractSlideExpandableListAdapter) slideExpandableListView.getAdapter();
			...
			} else if (slideAdapter.getParentInitialHeight() != -1) {
				this.mEndHeight = slideAdapter.getParentInitialHeight();
			} else {
				this.mEndHeight = slideAdapter.getSavedParentInitialHeight();
			}
		}
